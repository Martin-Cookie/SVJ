...
Aplikace SVJ Správa slouží k evidenci vlastníků, správě hlasování per rollam, hromadnému rozesílání dokumentů, evidenci plateb a kontrole dat. Běží lokálně na počítači výboru (není potřeba internet, kromě rozesílání emailů).
...
- Každá jednotka má přiřazený variabilní symbol (VS) — číslo, které vlastník uvádí při platbě
...
1. **Přesná shoda VS** — pokud variabilní symbol na platbě přesně odpovídá VS jednotky, platba se přiřadí automaticky (nejvyšší jistota)
2. **Shoda jména a částky** — pokud se jméno plátce shoduje s vlastníkem a částka odpovídá předpisu (střední jistota, vyžaduje potvrzení)
...
Nepřiřazené platby lze přiřadit ručně.
...
- **Vzorec:** `výsledek = (měsíční předpis × 12) + počáteční zůstatek − celkem zaplaceno`
...
**Variabilní symbol (VS).** Unikátní číslo přiřazené jednotce. Vlastník ho uvádí při platbě, aby aplikace poznala, za koho platba je. Je to hlavní způsob automatického párování.
...
## 5. Synchronizace s externími zdroji
...
## 10. Technické minimum pro výbor
...
| **Variabilní symbol (VS)** | Unikátní číslo pro identifikaci platby — vlastník ho uvádí při bankovním převodu |
